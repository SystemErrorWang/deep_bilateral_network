...
def padding(pytorch_layer):
    layer = LayerParameter_ncnn()
    layer.type = "Padding"
    #pad_l, pad_r, pad_t, pad_b
    #top	0	1	bottom	0	2	left	0	3	right	0
...
def AdaptiveMaxPooling(pytorch_layer):
    layer = LayerParameter_ncnn()
...
def AddConst(pytorch_layer):
    print('Using a dumb layer.')
    layer = LayerParameter_ncnn()
    layer.type = 'Power'
    layer.param.append('%f' % 1)
    layer.param.append('%f' % 1)
    """ Constant to add should be filled by hand, since not visible in autograd """
    #tmp = [7,7,8,7,8,6,5,7,7]
    tmp = [1E-16]
    layer.param.append('%f' % float(tmp.pop()))
    return layer
...
def eltwise(pytorch_layer):
    layer = LayerParameter_ncnn()
    layer.type = 'Eltwise'
    """ operation: 0=mul 1=add 2=max 3=div"""
    layer.param.append('%d' % 1)
    """  TODO: coefficient  """
    return layer
...
def eltwise_div(pytorch_layer):
    layer = LayerParameter_ncnn()
    layer.type = 'Eltwise'
    """ operation: 0=mul 1=add 2=max 3=div"""
    layer.param.append('%d' % 3)
    """  TODO: coefficient  """
    return layer
...
def sum(pytorch_layer):
    layer = LayerParameter_ncnn()
    layer.type = 'Reduction'
    layer.param.append('%d' % 0)
    if pytorch_layer.dim == 2:
        layer.param.append('%d' % 1)
    return layer


def unsqueeze(pytorch_layer):
    layer = LayerParameter_ncnn()
    layer.type = 'ExpandDims'
    layer.param.append('%d' % 0)
    if pytorch_layer.dim == 0:
        layer.param.append('%d' % 0)
        layer.param.append('%d' % 0)
        layer.param.append('%d' % 1)
    elif pytorch_layer.dim == 2:
        layer.param.append('%d' % 1)
        layer.param.append('%d' % 0)
        layer.param.append('%d' % 0)
    elif pytorch_layer.dim == 3:
        layer.param.append('%d' % 0)
        layer.param.append('%d' % 1)
        layer.param.append('%d' % 0)
    return layer
...
def expected_xy(pytorch_layer):
    layer = LayerParameter_ncnn()
    layer.type = 'ExpectedXy'
    return layer


def roi_align(pytorch_layer):
    layer = LayerParameter_ncnn()
    layer.type = 'RoiAlign'
    layer.param.append('%d' % pytorch_layer.crop_size[0])
    layer.param.append('%d' % pytorch_layer.crop_size[1])
    return layer


def clip(pytorch_layer):
    layer = LayerParameter_ncnn()
    layer.type = 'Clip'
    layer.param.append('%d' % 0)
    layer.param.append('%d' % 1)
...
def build_converter(opts):
    return {
        'NcnnRoiAlign': roi_align,
        'ExpectedXy': expected_xy,
        'data': data,
        'Addmm': inner_product,
        'Threshold': ReLU,
        'ConvNd': spatial_convolution,
        'MkldnnConvolution': spatial_convolution,
